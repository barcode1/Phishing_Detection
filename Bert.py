...
# ------------------- 2. لود دیتاست -------------------
df = pd.read_csv('data/malicious_phish.csv')  # اسم فایلت رو درست کن
df = df[['url', 'type']].dropna()

print("توزیع اولیه کلاس‌ها:")
print(df['type'].value_counts())

# ------------------- 3. تقسیم دیتاست (stratified) -------------------
...
# ------------------- 4. بالانس کردن فقط Train با Undersampling -------------------
...
print(f"\nمپینگ لیبل‌ها: {label2id}")

# ------------------- 6. پیش‌پردازش حرفه‌ای URL -------------------
...
#     logging_steps=10,
#
#     # فقط اینا کار می‌کنه در نسخه 4.57.3
#     eval_strategy="epoch",           # نه evaluation_strategy!
#     save_strategy="epoch",
...
#     dataloader_num_workers=0,        # روی ویندوز/جوپتر حتماً 0 باشه
#     torch_compile=False,
#     dataloader_pin_memory=False,
# )
training_args = TrainingArguments(
    output_dir='./url_malware_bert',
    num_train_epochs=4,                     # ۴ تا کافیه
    per_device_train_batch_size=16,         # ← خیلی مهم! از 128 به 16 کم کن
    per_device_eval_batch_size=32,          # ← اینم کم کن
    gradient_accumulation_steps=4,          # ← این باعث می‌شه effective batch_size = 16×4 = 64 بشه (عالی!)
    ...
    metric_for_best_model="f1_macro",
    greater_is_better=True,
    save_total_limit=2,                     # فقط یک بار!

    report_to="none",
    seed=42,
    remove_unused_columns=False,
    dataloader_num_workers=2,               # 2 یا 4 خوبه، بیشتر نزن
    dataloader_pin_memory=False,

    fp16=False,                             # روی CPU حتماً False
    gradient_checkpointing=True,            # ← این رم رو تا ۵۰٪ کم می‌کنه! (طلاییه)
    optim="adamw_torch",                    # پیش‌فرض خوبه
)
